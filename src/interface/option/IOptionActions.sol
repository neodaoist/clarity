...
interface IOptionActions {
    function writeNewCall(address baseAsset, address quoteAsset, uint32[] calldata exerciseWindow, uint256 strikePrice, uint64 optionAmount) external returns (uint256 optionTokenId);
    function writeNewPut(address baseAsset, address quoteAsset, uint32[] calldata exerciseWindow, uint256 strikePrice, uint64 optionAmount) external returns (uint256 optionTokenId);
